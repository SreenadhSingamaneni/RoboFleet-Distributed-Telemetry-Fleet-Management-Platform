...
public class KafkaConfig {
    ...
    ConcurrentKafkaListenerContainerFactory<String, AlertEvent>
            alertFanoutKafkaListenerContainerFactory(
        ...
        consumerProperties.put(
                ...
                "com.roboverse.fleet.infrastructure.messaging");
        consumerProperties.put(
                JsonDeserializer.USE_TYPE_INFO_HEADERS,
                false);

        var consumerFactory =
                new DefaultKafkaConsumerFactory<String, AlertEvent>(
                        consumerProperties);

        var factory =
                new ConcurrentKafkaListenerContainerFactory<String, AlertEvent>();
    ...
    NewTopic telemetryInvalidTopic(FleetProperties properties) {
        return TopicBuilder.name(properties.topics().telemetryInvalid())
                .partitions(3)
                .build();
    }
    ...
    NewTopic telemetryDeadLetterTopic(FleetProperties properties) {
        return TopicBuilder.name(properties.topics().telemetry() + ".dlt")
                .partitions(12)
                .build();
    }
    ...
    NewTopic alertsTopic(FleetProperties properties) {
        return TopicBuilder.name(properties.topics().alerts())
                .partitions(6)
                .build();
    }
}